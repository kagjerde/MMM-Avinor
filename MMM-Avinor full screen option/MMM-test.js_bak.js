...
Module.register("MMM-test", {
	...
	defaults: {
		...
		initialLoadDelay: 5000, // 2.5 seconds delay
		retryDelay: 2500, // retry delay
		animationSpeed: 5000, // animation speed
		httpRequestURL: "http://www.w3schools.com/xml/cd_catalog.xml"
	},
	...
	getDom: function() {
		...
		// Checks for results
			...
			if(this.requestComplete){
				...
				if (self.requestComplete===true) {
					wrapper.innerHTML = "Ferdig";				}
				wrapper.innerHTML = "Ikke ferdig..." + this.nodes.length + ":" + this.status + ":" + this.readyState + ":" + this.responseXML;
			}
		}
		else {
		...
		xhttp.onload = function() {
			if (this.readyState === 4) {   //this.readyState === 4
				if (this.status === 200) {		//this.status === 200
					...
					self.processData(this.responseXML); //XML
					self.updateDom(self.config.animationSpeed);
				}
				else {
